...
import {
  Users, Building2, FileText, AlertTriangle, BookOpen, CalendarRange,
  BarChart2, Settings, TrendingUp, CheckCircle2, Activity, type LucideIcon,
} from 'lucide-react';
...
const quickActions: ReadonlyArray<{ icon: LucideIcon; label: string; to: string; color: string }> = [
  { icon: FileText, label: 'Create Assessment', to: '/admin/assessments', color: '#3B82F6' },
    ...
    <main className="dashboard-main">
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-stats" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          ...
          {/* Attempt count (replaces the Languages card per NCFE Admin review) */}
          <div className="stat-card">
            ...
            <div className="stat-card__info">
              ...
              <div className="stat-card__value">1.18L</div>
              <div style={{ fontSize: '0.75rem', color: '#7C3AED', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                <TrendingUp size={11} aria-hidden="true" /> +18,400 this cycle
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px', marginTop: '28px' }}>
          ...
          <div className="dashboard-card">
            <div className="dashboard-card__header">
              <h3 className="dashboard-card__title">Pending Entity Verifications</h3>
              <Link href="/admin/entities" className="btn btn--sm btn--secondary" style={{ textDecoration: 'none', fontSize: '0.75rem' }}>
                ...
                <tbody>
                  {pendingEntities.map((e) => (
                    <tr key={e.id}>
                      ...
                      <td>
                        <Link href="/admin/entities" className="btn btn--sm btn--primary" style={{ textDecoration: 'none', fontSize: '0.75rem' }}>
                          Verify
                        </Link>
                      </td>
                    </tr>
          ...
          <div className="dashboard-card">
            ...
            <div className="dashboard-card__body" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {quickActions.map((item) => (
                <Link key={item.label} href={item.to} style={{ textDecoration: 'none', flex: '1 1 calc(50% - 6px)', minWidth: '120px' }}>
                  <div style={{ padding: '12px 14px', background: 'var(--gray-50)', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
        ...
        <div className="dashboard-card" style={{ marginTop: '24px' }}>
          <div className="dashboard-card__header">
            ...
            <Link href="/admin/audit-logs" className="btn btn--sm btn--secondary" style={{ textDecoration: 'none', fontSize: '0.75rem' }}>
              Full Audit Log
            </Link>
          </div>
